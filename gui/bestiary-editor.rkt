...
(define (main)
  ...
  (define (edit . xs)
    (match (car xs)
      ['remove-monster
       (match-define (list monster-set monster-name) (cdr xs))
       (<@ @info-db {(hash-update monster-set {(hash-remove monster-name)})})]
      ['stats
       (match-define (list* data f args) (cdr xs))
       (when data
            ...
            (λ (stats)
              ...
              (match* (f args)
                ...
                [{(== monster-stats-immunities) (list index value)}
                 (struct-copy monster-stats
                              stats
                              [immunities (list-set current-value index value)])]))))
         (<@ @info-db
             (λ (info-db)
               (hash-update
                info-db
                  ...
                  (hash-update
                   ...
                   (λ (info)
                     ...
                     (if elite?
                         ...
                         (struct-copy monster-info
                                      info
                                      [normal-stats (update-statss the-statss)])))))))))]
      ['abilities (void)]))
  (render
   (window
